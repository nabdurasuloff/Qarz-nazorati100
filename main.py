...
def yigma_jild_papka_yarat(mijoz_ism, anketa_raqami, mib_ish_raqami):
    """
    ...
    papka = os.path.join(YIGMA_JILD_DIR, papka_nomi)
...
def bajar_ommaviy_xat(parent, app, nomzodlar, boshqa_bosqich_soni=0):
    """
    Bir nechta mijoz uchun BIRDAN xat tayyorlab, darhol "Yuborildi" deb
    belgilaydi — foydalanuvchi PDF yoki Word formatini bir marta tanlaydi,
    keyin butun ro'yxat o'sha formatda ishlov ko'radi.

    nomzodlar: har biri {'portfel': ..., 'xat': ...(yoki None)} dict
    ro'yxati — 'xat' None bo'lsa, yangi xat yaratiladi; mavjud bo'lsa
    (hali yuborilmagan bo'lsa) — shunchaki "Yuborildi" deb belgilanadi.
    ...
    tasdiq_matni = (f"{len(nomzodlar)} ta mijoz uchun xat tayyorlanadi va darhol 'Yuborildi' "
                     f"deb belgilanadi ({format_tanlov} formatida).")
    ...
    yaratildi, otkazib_yuborildi, pdf_muvaffaqiyatsiz = 0, 0, 0
    xatolar = []
    for item in nomzodlar:
        prow = item['portfel']
        xat = item.get('xat')
        try:
            if not xat:
                ...
                conn = db.get_conn()
                row = conn.execute(
                    "SELECT id FROM xatlar WHERE anketa_raqami=? ORDER BY id DESC LIMIT 1",
                    (prow['anketa_raqami'],)
                ).fetchone()
                conn.close()
                if row:
    ...
    msg = f"{yaratildi} ta xat tayyorlandi va 'Yuborildi' deb belgilandi."
...
class LoginDialog(tk.Tk):
    ...
    def __init__(self):
        ...
        tk.Label(self, text="🏦", bg=NAVY_DARK, fg=GOLD, font=('Segoe UI', 32)).pack(pady=(28, 4))
        tk.Label(self, text="Qarz Nazorat va Talabnoma Tizimi", bg=NAVY_DARK, fg=WHITE,
                 font=('Segoe UI', 11, 'bold')).pack()
        tk.Label(self, text="Davom etish uchun parolni kiriting", bg=NAVY_DARK, fg=ICE,
                 font=('Segoe UI', 9)).pack(pady=(4, 14))

        self.parol_var = tk.StringVar()
        entry = tk.Entry(self, textvariable=self.parol_var, show='●', font=('Segoe UI', 12),
        ...
        tk.Label(self, textvariable=self.xato_var, bg=NAVY_DARK, fg='#FF8080',
                 font=('Segoe UI', 9)).pack(pady=(8, 0))

        btn = tk.Button(self, text="Kirish", command=self._tekshir, bg=GOLD, fg=NAVY_DARK,
...
class App(tk.Tk):
    def __init__(self, on_progress=None):
        ...
        tk.Label(splash, text="🏦 Qarz Nazorat va Talabnoma Tizimi", bg=NAVY_DARK, fg=WHITE,
                 font=('Segoe UI', 13, 'bold')).pack(pady=(28, 8))
        splash_status = tk.StringVar(value="Boshlanmoqda...")
        tk.Label(splash, textvariable=splash_status, bg=NAVY_DARK, fg=ICE,
                 font=('Segoe UI', 9)).pack(pady=4)
        splash_bar = ttk.Progressbar(splash, mode='determinate', length=320, maximum=100)
    ...
    def generate_bulk(self):
        ...
        for item in selected:
            ...
            try:
                ...
                fname = f"Davo_{turi}_{letters.safe_filename(xat['anketa_raqami'])}_{mijoz_ism_fayl}.docx"
...
class Nazorat95413Tab(ttk.Frame):
    ...
    def bajar_amal(self):
        ...
        if bosqich == 'xat_kerak':
            javob = messagebox.askyesno(
                "Tasdiqlash", f"{prow['mijoz_nomi']} uchun xat yaratib, darhol "
                              "'Yuborildi' deb belgilaymizmi?")
            if not javob:
                return
            path, pdf_xato, skipped = self.app.tab_talabnoma_xat._generate_for_row(prow)
            if skipped:
                ...
                if row:
                    db.mark_xat_yuborildi(row['id'])
                messagebox.showinfo("Bajarildi", "Xat yaratildi va 'Yuborildi' deb belgilandi.")
            self.refresh()
            return
...
class ChoraKorishTab(ttk.Frame):
    def __init__(self, parent, app):
        ...
        cols = ('anketa', 'mijoz', 'turi', 'dpd', 'jami_qarz', 'chora', 'tafsilot')
        self.tree = ttk.Treeview(self, columns=cols, show='headings', height=16)
        headings = {'anketa': 'Anketa №', 'mijoz': 'Mijoz', 'turi': 'Turi', 'dpd': 'DPD (kun)',
                    'jami_qarz': 'Qarzdorlik', 'chora': "Talab qilinayotgan chora", 'tafsilot': 'Tafsilot'}
        widths = {'anketa': 85, 'mijoz': 190, 'turi': 70, 'dpd': 70, 'jami_qarz': 110,
                  'chora': 220, 'tafsilot': 260}
        for c in cols:
            self.tree.heading(c, text=headings[c])
            self.tree.column(c, width=widths[c])
